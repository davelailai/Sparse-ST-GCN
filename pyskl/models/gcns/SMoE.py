...
class SMoEAssemble_sparse(nn.Module):
    def __init__(self,
        ...
        self.graph = Graph(**graph_cfg['expert'])
        ...
        for  i,(model_unit, sparse_unit) in enumerate(zip(model_list, sparse_ratio)): 
            ...
            if model_unit =='ST-GCN':
                ...
                models.append(STGCN_sparse(graph_cfg['expert'],in_channels=in_channels,linear_sparsity=sparse_unit,warm_up=warm_up,
                                           tcn_sparse_ratio=sparse_unit,gcn_sparse_ratio=sparse_unit,**ST_kwargs))
            if model_unit =='AA-GCN':
                AA_kwargs = kwargs['AA_kwargs']
                models.append(AAGCN_sparse(graph_cfg['expert'],in_channels=in_channels,linear_sparsity=sparse_unit,warm_up=warm_up,
                                           tcn_sparse_ratio=sparse_unit,gcn_sparse_ratio=sparse_unit,**AA_kwargs))
            if model_unit =='CTR-GCN':
                CTR_kwargs = kwargs['CTR_kwargs']
                models.append(CTRGCN_sparse(graph_cfg['expert'],in_channels=in_channels,linear_sparsity=sparse_unit,warm_up=warm_up,
                                            tcn_sparse_ratio=sparse_unit,gcn_sparse_ratio=sparse_unit,**CTR_kwargs))
            if model_unit =='DG-GCN':
                DG_kwargs = kwargs['DG_kwargs']
                models.append(DGSTGCN_sparse(graph_cfg['expert'],in_channels=in_channels,linear_sparsity=sparse_unit,warm_up=warm_up,
                                             tcn_sparse_ratio=sparse_unit,gcn_sparse_ratio=sparse_unit,**DG_kwargs))
        
        # assert gate_model[0] in ['ST-GCN', 'AA-GCN', 'CTR-GCN', 'DG-GCN']
        for model_unit in gate_model:
            assert model_unit in ['ST-GCN', 'AA-GCN', 'CTR-GCN', 'DG-GCN']
            Gate_kwargs=kwargs['Gate_kwarg']
            if model_unit =='ST-GCN':
                models.append(STGCN(graph_cfg['gate'],in_channels=in_channels,**Gate_kwargs))
            if model_unit =='AA-GCN':
                models.append(AAGCN(graph_cfg['gate'],in_channels=in_channels **Gate_kwargs))
            if model_unit =='CTR-GCN':
                models.append(CTRGCN(graph_cfg['gate'],in_channels=in_channels,**Gate_kwargs))
            if model_unit =='DG-GCN':
                models.append(DGSTGCN(graph_cfg['gate'],in_channels=in_channels, **Gate_kwargs))
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        threshold_positions_if_out = threshold_positions_if_in - self.num_experts1
