...
from .utils import bn_init, mstcn, unit_aagcn, unit_tcn, unit_aahgcn,unitmlp,msmlp
...
class AAGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, A, edge_type, node_type, stride=1, residual=True, **kwargs):
        ...
        assert gcn_type in ['unit_aagcn','unit_aahgcn']
...
class AAGCN(nn.Module):
    def __init__(self,
        ...
        if self.time_encoding:
            self.embed=unit_tcn(60, self.base_channels, kernel_size=1)
            self.tem_encoding = torch.tensor(self.one_hot(60))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            if self.time_encoding and i==0:
                # self.embed=embed(dim=T,dim1=self.base_channels)
                tem_encoding=self.tem_encoding.unsqueeze(0).unsqueeze(0)
                tem_encoding=tem_encoding.repeat(N*M,V,1,1)
                # tem_encoding = self.one_hot(N*M, T, V)
                tem_encoding=tem_encoding.permute(0,3,2,1).to(x.device)
                tem_encoding=self.embed(tem_encoding)
                x = self.gcn[i](x)
                x=x+tem_encoding
            else:
               x = self.gcn[i](x) 
